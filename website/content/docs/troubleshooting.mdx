...
Check that the server is running, the SSH tunnel is open, and the client points to the correct local port. The PyTorch device uses `RGPU_OPSERVER` and port 9720. The CUDA shim uses `RGPU_SERVER` and port 9713. The two protocols cannot connect to one another.
...
Pass `backend="rgpu"` explicitly, with `dynamic=False` for fixed-shape graph shipping. For diagnosis, use `backend=rgpu.compile_backend(compiler="eager")` to keep graph shipping while avoiding server-side Inductor compilation.
...
Start with `rgpu.stats()` with the PyTorch device. Look for repeated host reads, small batches, large uploads and compilation cost over a short run. Follow the [measurement checklist](/docs/performance/) before selecting an optimization.
