...
| --- | --- | --- |
| nanoGPT (`yangpt` variant), batch 64, context 256, 20 steps | 3.04 s/step local MPS opserver; 1.04 s/step remote A40 at 116 ms RTT | Both use rGPU; the local baseline is not native MPS |
...
These measurements used a local `yangpt` variant with modified settings, not the
pinned lecture script in the [nanoGPT example](/docs/nanogpt/). The recorded
compilation comparison does not establish a break-even point for longer runs.
