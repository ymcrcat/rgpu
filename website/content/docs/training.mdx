...
Complete the [PyTorch quickstart](/docs/quickstart/) first. Save the example
below as `train.py` and run it through `rgpu-run` using the same SSH options.
...
This is a small API example with synthetic data, not a performance benchmark.
For a transformer with a text dataset, see the [nanoGPT example](/docs/nanogpt/).
Downloading a loss every step adds a host wait. Decide how frequently you need metrics.
